...
function processRequest($rid) {
	...
	switch($rjs['type']) {
		...
		$stmt = $pdo->prepare("UPDATE `users` SET `dept` = :value, `rank` = 0 WHERE `id` = :uid");
...
function newArrest($crid, $crimes, $time, $date, $bailbond, $bail) {
	global $pdo;
	$bid = -1;
	if($bailbond == "Bond") {
	...
	$stmt = $pdo->prepare("INSERT INTO `arrests` (`uid` ,`copid` ,`docid` ,`date` ,`time` ,`bondid` ,`proc` ,`crimes` ,`bail` ,`plea` ,`exp` ,`dojid` ,`RealDate`) VALUES (:uid, :copid, '0', :date, :time, :bondid, '0', :crimes, :bail, '0', '0', '0', UTC_TIMESTAMP())");
...
function cancelBolo($boid) {
	...
	$stmt = $pdo->prepare("UPDATE `bolo` SET `canceled` = :copid WHERE `id` = :id");
...
function getDepts($auth = 1000, $inorder = false, $section = 0) {
	global $pdo;
	$stmt = NULL;
